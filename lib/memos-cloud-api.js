...
export function buildConfig(pluginConfig = {}) {
  ...
  const rumEnabled = parseBool(cfg.rumEnabled, parseBool(loadEnvVar("MEMOS_RUM_ENABLED"), false));
  const rumEndpoint = normalizeEndpoint(cfg.rumEndpoint ?? loadEnvVar("MEMOS_RUM_ENDPOINT") ?? "");
  const rumEnv = cfg.rumEnv ?? loadEnvVar("MEMOS_RUM_ENV") ?? "prod";
  const rumService = cfg.rumService ?? loadEnvVar("MEMOS_RUM_SERVICE") ?? "";
  const rumTimeoutMs = parseNumber(cfg.rumTimeoutMs ?? loadEnvVar("MEMOS_RUM_TIMEOUT_MS"), 3000);
